...
def render(md):
    ...
    return str(s)
def main():
    d=json.loads((P/'poti-podatki.json').read_text(encoding='utf-8'));r=d['poti'];ns=d['naselja'];stats=d['statistika']
    aed=json.loads((P/'aed.json').read_text(encoding='utf-8'))
...
'''
    ...
    out=f'''<section class="section" id="poti" data-title="Pohodne in kolesarske trase"><style>{(P/'poti.css').read_text(encoding='utf-8')}</style>
...
<div class="poti-searchbox"><label>Iskanje trase <input id="poti-search" type="search" placeholder="Npr. Grički kal, Vinica, Mirna gora"></label><a class="btn" href="poti.csv" download>Trase CSV</a><a class="btn" href="poti-pokritost-naselij.csv" download>Oddaljenost krajev CSV</a><a class="btn" href="poti.geojson" download>GeoJSON</a><a class="btn" href="poti-zemljevid.svg" download>Zemljevid SVG</a></div>
...
<script type="application/json" id="poti-data">{encoded_data}</script><script>{(P/'poti.js').read_text(encoding='utf-8')}</script></section>'''
    # f-string je pri dvojnih oklepajih namenoma uporabljen le za statični del.
    out=re.sub(r'(?<!\{)\{cite:([^}]+)\}(?!\})',r'{{cite:\1}}',out)
    (P/'poti.html').write_text(out,encoding='utf-8')
    standalone=svg.replace('<svg ', '<svg id="poti" ',1).replace('<title>','<style>'+(P/'poti.css').read_text(encoding='utf-8').replace('#poti ','')+'</style><title>',1)
    (P/'poti-zemljevid.svg').write_text(standalone,encoding='utf-8')
...
def write_wiki(d,mainmd,intro,summary,coverage,gaps,suitability,methodology,solutions):
    raw=ROOT/'raw/web/poti-gpx-20260921-manifest.md'
    manifest=json.loads((P/'poti-manifest.json').read_text(encoding='utf-8'))
    if not raw.exists():raw.write_text('# Predložena zbirka poti GPX\n\nDatum obdelave: 2026-09-21. Vir: naročnik; 43 izvirnih datotek v `porocila/zdravstvo-obcina-crnomelj/viri/gpx/`. Izvorni avtorji in datum terenske preverbe niso potrjeni. To ni spletni zajem.\n\n'+table(['Datoteka','SHA-256'],[[t['datoteka'],t['sha256']] for t in manifest['datoteke']]),encoding='utf-8')
    osmraw=ROOT/'raw/web/osm-crnomelj-poti-20260921.md';dest=ROOT/'raw/web/priloge/osm-crnomelj-20260920.json'
    ...
    (P/'poti-viri.json').write_text(json.dumps(refs,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    literature='''## Literatura

[1] »Predložena zbirka 43 poti GPX,« naročnik poročila, obdelava 21. septembra 2026. Lokalni izvirniki: `porocila/zdravstvo-obcina-crnomelj/viri/gpx/`; [[../../../raw/web/poti-gpx-20260921-manifest|manifest s kontrolnimi vsotami]]. Izračuni: [[../../../porocila/zdravstvo-obcina-crnomelj/poti-podatki.json]], [[../../../porocila/zdravstvo-obcina-crnomelj/poti-pokritost-naselij.csv]].

[2] »Meja Črnomlja in točke krajev,« sodelavci OpenStreetMap, 20. september 2026. [Na spletu]. Dostopno: https://www.openstreetmap.org/relation/1685747. [Dostopano: 20. september 2026]. Lokalna kopija: [[../../../raw/web/osm-crnomelj-poti-20260921]]. © sodelavci OpenStreetMap, ODbL.
'''
