...
def sources():
    refs={r['key']:r for r in json.loads((P/'literatura-zajem.json').read_text(encoding='utf-8'))['records']}
    extra=P/'starost-viri.json'
    if extra.exists():refs.update(json.loads(extra.read_text(encoding='utf-8')))
    extra=P/'drustva-viri.json'
    if extra.exists():refs.update(json.loads(extra.read_text(encoding='utf-8')))
    extra=P/'kader-sport-viri.json'
    if extra.exists():refs.update({'kader-'+k:v for k,v in json.loads(extra.read_text(encoding='utf-8')).items()})
    extra=P/'poti-viri.json'
    if extra.exists():refs.update(json.loads(extra.read_text(encoding='utf-8')))
    ...
    refs['nizi']={'title':'Časovni nizi kazalnikov za Črnomelj, izdaje 2016–2026: 54 Excelovih datotek','publisher':'NIJZ','date':'2026','url':'https://obcine.nijz.si/obcine/crnomelj/17/2026/','local':'raw/Zdravje_kraja','note':'Lokalni izvirniki XLSX; izvoz v kazalniki.csv. Leto v nizu pomeni leto objave.'}
    refs['opisi']={'title':'Zdravje v občini 2026 — Opisi kazalnikov','publisher':'NIJZ','date':'marec 2026','url':'https://obcine.nijz.si/vsebine/metodoloska-pojasnila/opisi-kazalnikov-2026-223/','local':'raw/Zdravje_kraja/Opisi_kazalnikov.pdf','note':'Prebran lokalni izvirnik: str. 8–9, 19–20, 66–67, 96–97 in 108–109 (natisnjene strani).'}
    ...
    for key,pmid,authors,title,journal,year,vol,no,pages,doi in papers:
        refs[key]={'key':key,'title':title,'authors':authors,'publisher':journal,'date':str(year),'volume':vol,'number':no,'pages':pages,'doi':doi,'url':f'https://pubmed.ncbi.nlm.nih.gov/{pmid}/','local':f'raw/web/zdravstvo-20260919-pubmed-{pmid}.md','note':'Prebran strukturirani povzetek izvirne raziskave; ne celotno besedilo.'}
    refs['hall']['local']='raw/web/zdravstvo-20260919-hall-2019-povzetek.md'
    ...
    odeon=json.loads((P/'zajem-odeon.json').read_text(encoding='utf-8'))['records']
...
def citation(ref,n):
    ...
    local=f' Lokalni vir: <a href="../../{E(ref["local"])}">odpri shranjeno gradivo</a>.'
...
def load_data():
    all_data={};csvrows=[]
    for p in sorted((ROOT/'raw/Zdravje_kraja').glob('NIJZ*.xlsx')):
    ...
    with (P/'kazalniki.csv').open('w',encoding='utf-8-sig',newline='') as f:
        w=csv.writer(f,delimiter=';');w.writerow(['sifra','kazalnik','obmocje','leto_objave','vrednost_izvirnika']);w.writerows(csvrows)
    (P/'kazalniki.json').write_text(json.dumps(all_data,ensure_ascii=False,indent=2),encoding='utf-8')
    return all_data
...
def main():
    ...
    rows=process();stats=json.loads((P/'statistika-registra.json').read_text(encoding='utf-8'));refs=sources();data=load_data()
    assert len(data)==54,len(data)
    body=(P/'vsebina.html').read_text(encoding='utf-8')
    if '{{demografija}}' in body:body=body.replace('{{demografija}}',(P/'demografija.html').read_text(encoding='utf-8'))
    if '{{drustva}}' in body:body=body.replace('{{drustva}}',(P/'drustva.html').read_text(encoding='utf-8'))
    if '{{aed}}' in body:body=body.replace('{{aed}}',(P/'aed.html').read_text(encoding='utf-8'))
    if '{{kader-sport}}' in body:body=body.replace('{{kader-sport}}',(P/'kader-sport.html').read_text(encoding='utf-8'))
    if '{{poti}}' in body:body=body.replace('{{poti}}',(P/'poti.html').read_text(encoding='utf-8'))
    ...
    registry(rows,stats)
    (P/'literatura.json').write_text(json.dumps([{**refs[k],'key':k,'number':i} for i,k in enumerate(order,1)],ensure_ascii=False,indent=2),encoding='utf-8')
