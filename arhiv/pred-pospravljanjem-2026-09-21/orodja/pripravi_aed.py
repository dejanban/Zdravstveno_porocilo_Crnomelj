"""Zemljevid defibrilatorjev: viri/AEDs.xlsx + viri/osm-crnomelj.json -> aed.json, aed.csv, aed.html (razdelek poročila)."""
...
def podatki():
    ...
    (P / 'aed.json').write_text(json.dumps(d, ensure_ascii=False), encoding='utf-8')
    with (P / 'aed.csv').open('w', encoding='utf-8-sig', newline='') as f:
...
<div class="aed-filter"><label>Iskanje<input type="search" id="aed-search" placeholder="Npr. Vinica, šola, gasilski"></label> <a class="btn" href="aed.csv" download>Prenesi CSV</a></div>
...
<script>{JS}</script>
    ...
    (P / 'aed.html').write_text(out, encoding='utf-8')
