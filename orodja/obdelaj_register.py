...
def process():
    d=json.loads((P/'podatki/register/zajem-odeon.json').read_text(encoding='utf-8')); rows=[]
    ...
    (P/'podatki/register/register-dogodkov.json').write_text(json.dumps(rows,ensure_ascii=False,indent=2),encoding='utf-8')
    with (P/'podatki/register/register-dogodkov.csv').open('w',encoding='utf-8-sig',newline='') as f:
    ...
    (P/'podatki/register/statistika-registra.json').write_text(json.dumps(stats,ensure_ascii=False,indent=2),encoding='utf-8')
