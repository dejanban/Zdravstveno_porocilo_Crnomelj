...
def main():
    ...
    (P/'podatki/poti/poti-podatki.json').write_text(json.dumps(result,ensure_ascii=False),encoding='utf-8')
    (P/'podatki/poti/poti.geojson').write_text(json.dumps({'type':'FeatureCollection','features':features},ensure_ascii=False),encoding='utf-8')
    (P/'podatki/poti/poti-manifest.json').write_text(json.dumps({'datum':'2026-09-21','datoteke':manifest,'metoda':result['metoda']},ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    for filename,rows in [('podatki/poti/poti.csv',manifest),('podatki/poti/poti-pokritost-naselij.csv',ns)]:
